  ...
  return (
    <SectionBackground className="flex min-h-screen flex-col items-center justify-center px-4 py-16 text-center">
      ...
      >
        ...
        {/* Contact Cards */}
        <motion.div className="mt-4 w-full grid grid-cols-1 sm:grid-cols-3 gap-4"
          variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }}
        >
          {[
            ...
            >
              <motion.div className="absolute -inset-[1px] rounded-xl"
                ...
                transition={{ duration: 2, repeat: Infinity }}
              />
              <div className="relative rounded-xl px-4 py-4 flex flex-col items-center gap-2 backdrop-blur-sm overflow-hidden" style={cardStyle}>
                {/* Stars & bubbles inside contact cards */}
                {contactStars.map(s => <CardStar key={s.id} x={s.x} y={s.y} size={s.size} delay={s.delay} />)}
                {contactBubbles.map(b => <CardBubble key={b.id} x={b.x} size={b.size} delay={b.delay} dur={b.dur} />)}
                <motion.span className="text-2xl relative z-10"
                  animate={hoveredContact === item.key ? { scale: [1, 1.3, 1], rotate: [0, 10, -10, 0] } : {}}
                  transition={{ duration: 0.5 }}
                >{item.icon}</motion.span>
                <span className="font-body text-sm font-medium relative z-10" style={{ color: "hsl(0 60% 25%)" }}>{item.label}</span>
              </div>
            </motion.a>
          ))}
        </motion.div>
